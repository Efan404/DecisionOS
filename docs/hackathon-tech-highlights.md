...
**Stage-Aware Query Rewriting（阶段感知查询改写）**

检索前先对 `idea_seed` 做阶段感知改写——纯字符串拼接，零延迟，零 LLM 调用——将检索锚定在当前工作流阶段的语义焦点上：

```python
# context_loader.py — _rewrite_query（实际代码）
_STAGE_FOCUS = {
...
改写后的 query 用于三路检索，确保在 feasibility 阶段优先召回竞品/商业模式相关记忆，而不是在 PRD 阶段召回到泛泛的市场机会描述：
...
evidence      = retrieve_market_evidence_context(query=retrieval_query)  # 最多 800 tokens
```
...
**`strategy_router` 节点：规则推理，在生成前自主决定方案优先级**

```python
# strategy_router.py — 零 LLM 调用，关键词评分决定商业路径推荐（实际代码）
_RULES = [
    (["marketplace", "platform", "ecosystem", "partner"], "platform"),
    (["saas", "b2b", "enterprise", "viral", "growth"],     "vc_funded"),
    (["tool", "indie", "developer", "cli", "productivity"], "bootstrapped"),
]

scores = {"bootstrapped": 0, "vc_funded": 0, "platform": 0}
for keywords, strategy in _RULES:
    hits = [kw for kw in keywords if kw in idea_seed.lower()]
    if hits:
        scores[strategy] += len(hits)

# 无信号时默认 bootstrapped（最保守假设）
tiebreak = {"bootstrapped": 2, "vc_funded": 1, "platform": 0}
recommended = max(scores, key=lambda s: (scores[s], tiebreak[s]))
```

`strategy_router` 的输出（`recommended_strategy`）写入 State，下游 `plan_generator` 读取后把推荐的商业路径排到第一个生成，推荐结果和评分过程通过 `agent_thought` SSE 事件实时透传到前端——决策链的每一步对用户完全可见。
...
| 决策路由   | Strategy Router（规则引擎，零 LLM 调用）          | 关键词评分 → 推荐商业路径 → 重排生成顺序，决策过程透明 |
| RAG 检索   | ChromaDB（4 Collections）+ Stage-Aware Rewriting  | 阶段感知改写 query，4 阶段各自语义焦点，零额外延迟     |
| LLM 调用   | ai_gateway.py（OpenRouter / Anthropic）           | 三重防幻觉 + 括号匹配 JSON 提取 + 2 次重试             |
| 向量存储   | ChromaDB（余弦相似度 + Hybrid 复合评分）          | 语义同质分 Collection + 竞品/信号关系增益叠加          |
